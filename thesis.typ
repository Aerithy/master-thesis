...
#show: thesis.with(
  ...
  achievement: [
    ...
    [2] 肖利民，沈润楠，王锦权，霍志胜，谭升阳，*李云潼*，阮利. 面向云边端协同环境的数据布局方法[P]. 中国专利: CN119544721A, 2025-02-28.

    [3] Runnan Shen, Jinquan Wang, Zhisheng Huo, Limin Xiao, Shengyang Tan, *Yuntong Li*, Lun Li, Xiangrong Xu, Liang Wang. A two-stage data placement strategy for cloud-edge-device collaborative environment[J]. COMPUTER COMMUNICATIONS, 2026-04-27

    == 攻读硕士学位期间参与的主要科研工作

    [1] 参与面向云边端协同的芯粒结构与系统软件（项目编号：2023YFB4503100）.国家重点研发计划共性关键技术类专项. 2023.11——2026.10. 作为项目/课题骨干，参与项目的调研、申请、实施阶段，主要围绕云边端跨域分布式训练场景下的通信瓶颈问题开展了系统性研究，负责云边端跨域分布式训练的通信优化技术方案设计与实现等工作.
  ],
  acknowledgements: [
    ...
    其次，我要感谢实验室的同学们对我的关心和帮助。非常感谢王锦权博士对我在科研工作和工程实践上的指导和讨论，锦权学术能力比起我们都要强上不少，让我十分钦佩。在和锦权一同完成的各项科研工作中，我们相互之间的讨论和分析给我提供了许多帮助，教会了我许多工程技术上的学习技巧，也获取了很多学术上的灵感。同时也要感谢我的同门谭升阳同学，和他在研究生伊始共同求学时便是我学习的榜样，他的代码能力和工程经验在我的开发过程和求职过程中给予了很大帮助。同样也要感谢贾志斌师兄和沈润楠师兄，两位师兄为我的学术探索提供了宝贵的思路。感谢各位同窗好友们，与你们共度的时光充满欢乐和收获。
  ...
  conclusion: [
    ...
    （1）针对跨域受限链路下数据交换载荷过大导致的训练吞吐受限问题，提出分布式优化器低比特量化方法。首先，通过 k-bit 随机舍入量化分布式压缩机制，将连续浮点梯度映射为低比特表示的有限的离散数值；其次，通过带有残差补偿的量化同步与低位聚合算法，以缓解位宽降低带来的累积噪声，并优化了单次同步的通信带宽开销。实验证明，该方案在低带宽场景下显著降低了梯度通信同步开销，且加速优势随网络受限程度逐渐增强。基于随机舍入量化，该方案在受限网络下可大幅压缩梯度通信量，并可保障收敛稳定性；通过低位宽全归约方法，该方案克服了低位宽数据和高效集合通信方法不兼容的问题。

    （2）针对跨域链路中跨域链路异构且域间网络带宽低导致的训练中集合通信执行效率低的问题，提出基于流水线的层次化梯度通信调度策略。首先，通过引入分层集合通信算法，将全局各节点的全归约通信分解为域内归约、跨域传输与域内广播三个阶段，降低了域间通信量；其次，通过引入张量分块级的流水线机制，将域内规约、跨域传输与域内广播三阶段按切分的张量块流水执行，优化了跨域链路间通信时域内链路的利用率。实验证明，该方案优化了跨域异构集群间数据聚合的调度重叠率，充分利用了域内部的高速带宽。通过引入分层集合通信算法，降低了原始集合通信在云边端跨域场景下在域间的通信时间，大幅消除了通信启动延迟。通过引入张量块流水执行，并行了域间和域内通信，降低了原始分层集合通信算法的整体开销。

    （3）针对跨域训练在单次迭代末尾极易产生不可掩盖的阻塞式同步尾延迟问题，提出面向通信受限场景下的混合并行通信掩盖策略。首先，通过在反向传播阶段提前触发非阻塞集合通信，为梯度同步的长尾部预留足够的计算掩盖窗口；其次，利用张量缩放策略，来预测推算尚未计算完成的梯度数据并将其纳入跨域梯度同步，降低由于提前触发通信导致的梯度数据尺度不一致的问题；最后，利用轻量级误差反馈，将预测梯度值与剩余梯度的偏差延迟补偿至下一计算迭代，去除了由于使用仅部分数据的梯度信息产生的信息损失。实验证明，该方案优化了设备本地计算时间与跨域通信时长的掩盖比率，最终成功消除了由跨域 All-Reduce 造成的尾部纯等待空窗，且维持了每迭代一次全局同步的语义。

    == 下一步工作展望

    本文解决了在云边端跨域分布式训练通信优化中的三类关键问题，优化了跨域训练通信效率，提升了跨域训练的整体性能。但这些方法仍具有一定的改进空间，其中主要包含以下三方面：

    （1）针对低比特优化器量化方法，本文采用了基于块量化的随机舍入量化策略和量化误差补偿技术来保证训练精度，但是在实际应用中，针对不同模型结构、训练任务和数据分布，单一的量化策略并不总是最优解。固定的块大小可能无法始终保持着最优精度解，依据如下。对于不同的模型结构、训练任务和数据分布，梯度数值的分布和异常值频率很有可能不一样。对较大的异常值频率而言，较小的块大小可能更有利于捕捉异常值，从而保持训练精度，但提升了传输的数据量；而对于较小的异常值频率而言，较大的块大小可能更有利于提高压缩率，但提升了当某块出现异常值时对该块总体量化数值影响的规模。因此，未来的工作可以考虑引入动态块量化方法，根据模型结构、训练任务和数据分布的不同，动态调整块大小，得到一个最优块粒度取值。提升训练精度的同时，以进一步提升量化优化器的性能。其次，本文的低比特全归约方法还可进一步提升效率，通过重写NCCL All-Reduce Kernel，将All-to-all和本地反量化规约操作融合，实现通算融合。由于All-to-all通信的特点是每个节点都会依次收到来自其他节点的部分数据，因此在接收数据的同时就可以进行反量化规约操作，避免了先完成All-to-all通信再进行反量化规约的两阶段过程，从而进一步提升通信效率。

    （2）针对基于流水线的层次化通信调度策略，本文提出了基于张量切分的流水线机制和双流并行执行来提升跨域链路的利用率，但在实际应用中，训练节点间的通信拓扑结构可能更加复杂，正像本文所述，不同的链路结构需要不同的通信算法来达成最好的性能表现。仅仅将云边端链路解构为域内和域间来分析，没有结合更细粒度的拓扑感知是这一方法的缺点。因此，未来的工作可以考虑引入更细粒度的拓扑感知机制，来适应更复杂的通信拓扑结构。通过对训练节点间的通信拓扑进行更细粒度的分析和建模，识别出不同链路之间的性能差异和瓶颈，进行通信算法的微调。从而设计出更加适应实际通信拓扑结构的调度策略。提升跨域链路的利用率，进一步提升跨域训练的整体性能。

    （3）针对混合并行通信掩盖策略，本文提出了在每步一次全局同步的语义下，将 All-Reduce 触发点前移并引入预测误差修正来消减迭代尾部纯等待。在实际应用中，大型模型的训练实现范式已经偏向于 Megatron、DeepSpeed 的整体架构，但我们的工作仅支持在 PyTorch 上进行训练，没有兼容更多的并行策略，例如张量并行、序列并行和上下文并行等。因此，未来的工作主要考虑将我们的方法适配进入 Megatron，从而支持各类大模型训练的并行策略，将本方法在业界真正落地，在更大规模的模型训练中验证其有效性，并扩展至真实预训练任务中验证其有效性。
  ]
  // cv: [
