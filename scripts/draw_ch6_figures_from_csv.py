...
def draw_ch6_system_performance_compare() -> None:
    ...
    ax2.plot(df["network_group"], df["baseline_comm_share"], marker="s", linewidth=2.0, color="#E8684A", label="通信占比-基线")
    ax2.plot(df["network_group"], df["system_comm_share"], marker="s", linewidth=2.0, color="#5B8FF9", label="通信占比-完整")
    ax2.set_ylabel("迭代时间（ms）/通信占比（%）", fontsize=16)
    ax2.set_title("迭代时间与通信占比", fontsize=18)
