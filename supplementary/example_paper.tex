...
\twocolumn[
  \icmltitle{POLAR-SGD: Efficient Cross-Datacenter Training via Pipelined Communication and Predictive Error Correction}
...
\begin{abstract}
% 中文注释：训练万亿参数级别的大型语言模型（LLMs）通常需要跨越多个地理分散的数据中心（Cross-DC）进行，并采用数据并行（DP）和流水线并行（PP）的混合策略。然而，在跨数据中心环境中，这种DP+PP范式面临着严峻挑战：广域网（WAN）的低带宽（10-20 Gb/s）和高延迟（30-100ms）使得用于梯度同步的阻塞式All-Reduce操作成为关键瓶颈，导致GPU产生50-70%的空闲时间。
Training trillion-parameter Large Language Models (LLMs) increasingly requires distribution across geographically dispersed datacenters (Cross-DC) using a hybrid of Data Parallelism (DP) and Pipeline Parallelism (PP). However, in Cross-DC environments, this DP+PP paradigm faces a critical challenge: Wide Area Networks (WAN) with low bandwidth (10-20 Gb/s) and high latency (30-100ms) make the blocking All-Reduce operation for gradient synchronization a severe bottleneck, causing 50-70\% GPU idle time.

% 中文注释：为解决这一问题，我们提出了POLAR-SGD，一种新颖的训练方法，它通过将通信与计算重叠来消除同步瓶颈。POLAR-SGD的核心是将每个mini-batch内的micro-batch后向传播视为一个局部步骤，并立即为该步骤的梯度启动一个非阻塞的All-Reduce操作。这使得梯度通信可以与下一个micro-batch的计算并行进行。为了解决这种异步性引入的梯度偏差问题，POLAR-SGD集成了一个轻量级的预测性纠错机制，该机制可以补偿梯度陈旧性，从而在不牺牲模型收敛性的前提下保持训练的稳定性。
To address this, we propose POLAR-SGD, a novel training method that eliminates the synchronization bottleneck by overlapping communication with computation. The core idea of POLAR-SGD is to treat the backward pass of each micro-batch within a mini-batch as a local step and immediately initiate a non-blocking All-Reduce for its gradients. This allows gradient communication to proceed in parallel with the computation of the next micro-batch. To resolve the gradient divergence introduced by this asynchrony, POLAR-SGD incorporates a lightweight predictive error-correction mechanism that compensates for gradient staleness, thereby maintaining training stability without sacrificing convergence.

% 中文注释：我们在一个包含32个A100 GPU的集群上对LLaMA-2 7B等模型进行了评估，并通过网络限流模拟真实的跨数据中心条件（20 Gb/s带宽，50ms延迟）。实验结果表明，在这种低带宽、高延迟的网络环境下，与标准的DP+PP基线相比，POLAR-SGD实现了1.87倍的训练吞吐量提升，同时将通信导致的GPU空闲时间从34%降低到7%（4.8倍的减少）。重要的是，这种加速并未以牺牲模型质量为代价，POLAR-SGD的收敛曲线与同步基线几乎完全一致。我们的消融研究进一步证实，预测性纠错机制对于防止异步通信引起的模型发散至关重要。POLAR-SGD为跨数据中心的大规模LLM训练提供了一个实用且高效的解决方案。
We evaluated POLAR-SGD on models including LLaMA-2 7B on a cluster of 32 A100 GPUs, emulating real Cross-DC conditions (20 Gb/s bandwidth, 50ms latency) via network throttling. Experiments show that under these low-bandwidth, high-latency network conditions, POLAR-SGD achieves a \textbf{1.87×} improvement in training throughput compared to a standard DP+PP baseline, reducing GPU idle time due to communication from 34\% to 7\% (a 4.8× reduction). Critically, this acceleration comes with no loss in model quality; POLAR-SGD's convergence curve is nearly identical to the synchronous baseline, which offers a practical and efficient solution for Cross-DC large-scale LLM training.
\end{abstract}
    ...
    \input{section/1_background}

    % \section{Motivation}
    % \input{section/motivation}

    \section{\sysname Design}
    \input{section/2_design}

    \section{Implementation}
    \label{implementation}

    \input{section/3_implementation}    
    ...
    % 中文注释：我们的工作建立在分布式训练、通信优化和异步SGD等领域的大量现有研究之上。
    Our work builds upon a large body of existing research in distributed training, communication optimization, and asynchronous SGD.

    \textbf{Communication-Efficient SGD.} 
    % 中文注释：为了减少分布式训练中的通信开销，研究人员探索了多种策略。梯度压缩技术，如量化（例如，QSGD）和稀疏化（例如，gTop-k），通过减少每次传输的数据量来降低通信时间。然而，这些方法通常是“有损的”，可能会影响收敛性，并且它们只是缩短了通信延迟，而没有消除阻塞式同步点本身。
    To reduce communication overhead in distributed training, various strategies have been explored. Gradient compression techniques, such as quantization and sparsification, lower communication time by reducing the amount of data transmitted per transfer. However, these methods are often lossy, can impact convergence, and merely shorten the communication stall rather than eliminating the blocking synchronization point itself.

    \textbf{Asynchronous and Semi-Asynchronous SGD.} 
    % 中文注释：完全异步的SGD（Async-SGD）完全消除了同步等待，但由于梯度陈旧性问题，通常会导致收敛性差。Local SGD（也称为Federated Averaging或Parallel SGD）通过在多次本地更新后进行一次同步，来摊销通信成本。这在减少通信频率方面是有效的，但它并没有解决单次同步操作的阻塞性质。此外，增加本地步骤的数量会增加模型发散的风险。POLAR-SGD通过将通信流水化并结合误差补偿，直接解决了这种发散问题，从而允许更积极的通信重叠。
    Fully asynchronous SGD (Async-SGD) removes synchronization waits entirely but often suffers from poor convergence due to gradient staleness. Local-update methods (often referred to as Local SGD or federated averaging) amortize communication cost by performing a single synchronization after multiple local updates. While effective at reducing communication frequency, they do not address the blocking nature of the synchronization itself. Furthermore, increasing the number of local steps raises the risk of model divergence. POLAR-SGD directly addresses this divergence by pipelining communication with error feedback, allowing for aggressive communication overlap.

    \textbf{Communication Overlap.} 
    % 中文注释：在标准的DDP中，已经存在一些与后向传播计算重叠通信的尝试。然而，这种重叠仅限于单个后向传播过程内部，并且在All-Reduce操作的最后部分，流水线仍然会停顿。一些工作，如Baek et al. 的研究，尝试在流水线并行中重叠梯度更新，但没有像我们一样将其推广到DP+PP设置中，并系统地解决随之而来的梯度偏差问题。我们的工作通过将每个micro-batch视为一个局部步骤，并将误差补偿理论应用于这种细粒度的异步模型，从而将重叠提升到了一个新的水平。
    Some efforts to overlap communication with backpropagation computation already exist in standard DDP. However, this overlap is confined within a single backward pass, and the pipeline still stalls on the final portion of the All-Reduce. Prior overlap techniques in pipeline-parallel settings typically do not generalize cleanly to the DP+PP setting or address the resulting gradient divergence. Our work takes overlap to the next level by treating each micro-batch as a local step and applying error-feedback theory to this fine-grained asynchronous model.

    \section{Conclusion}
    \label{conclusion}

    % 中文注释：在本文中，我们提出并评估了POLAR-SGD，一种旨在解决大规模LLM训练中DP+PP混合并行范式下通信瓶颈的新方法。通过将阻塞式的梯度同步转化为与计算重叠的流水线式通信，并集成一个预测性纠错机制来补偿异步性，POLAR-SGD在不牺牲模型收敛质量的前提下，显著提升了端到端的训练吞吐量。
    In this paper, we proposed and evaluated POLAR-SGD, a novel method designed to tackle the communication bottleneck in the DP+PP hybrid parallelism paradigm for large-scale LLM training. By transforming blocking gradient synchronization into a pipelined communication process that overlaps with computation, and by integrating a predictive error-correction mechanism to compensate for asynchrony, POLAR-SGD significantly improves end-to-end training throughput without sacrificing model convergence quality.

    % 中文注释：我们的实验结果表明，POLAR-SGD能够将在LLaMA-2 7B等模型上的训练速度提升1.87倍，同时保持与同步基线相当的困惑度。这项工作证明了，通过精心设计的算法，可以有效地隐藏分布式训练中的通信延迟，从而更高效地利用大规模计算集群。我们相信，POLAR-SGD为未来在日益增长的计算资源上训练更强大的语言模型铺平了道路。
    Our experimental results demonstrate that POLAR-SGD can accelerate training on models like LLaMA-2 7B by up to 1.87× while maintaining perplexity on par with the synchronous baseline. This work proves that it is possible to effectively hide communication latency in distributed training with a carefully designed algorithm, leading to more efficient utilization of large-scale compute clusters. We believe POLAR-SGD paves the way for training ever-more-powerful language models on growing computational resources more efficiently.
