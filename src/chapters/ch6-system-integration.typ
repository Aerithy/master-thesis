...
#figure(
  table(
    ...
    stroke: none,
    table.hline(),
    table.header([*网络分组*], [*链路条件*], [*现有训练系统吞吐*], [*本系统吞吐*], [*加速比*], [*TTT：现有/本系统（min）*], [*最终验证损失*]),
    table.hline(stroke: 0.5pt),
...
#figure(
  ...
  caption: [固定模型与 batch 条件下，本系统与基线系统的吞吐、迭代时间与通信占比对比]
) <fig:ch6-system-perf>

如@fig:ch6-system-perf 所示，在固定模型与 batch 设置下，本系统在 弱受限/中度受限/强受限 网络区间均获得吞吐提升，同时迭代时间下降、通信占比降低，验证了“性能测试”这组实验结论。

从吞吐看，网络约束越强，组合优化收益越明显：弱受限 为 1.18×，中度受限 为 1.46×，强受限 为 1.87×。这一趋势说明三模块协同在“通信主导”区间更能发挥叠加效果。
...
如@fig:ch6-system-degradation 所示，在高带宽或低 RTT 条件下，本系统加速比收敛至接近 1，同时回退比例显著提高且最终损失差值维持低水平，说明系统可自动回退并保持与基线一致行为，满足“退化测试”验证目标。
