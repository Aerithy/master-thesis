...
在跨域训练的环境中，以流水线并行为骨架会迫使训练样本/激活在域间频繁穿梭，入口与跨段链路容易成为瓶颈，如@fig:cross-dc-pp 所示 @crosspipe2025。
...
面对同步尾部，一类直观做法是弱化同步（如 Local-SGD），该类做法通过在各个计算域内部维护一个局部模型，并通过约定好的同步频率来维护各个域的模型参数一致，但这类方法的本质是降低同步频率而非消除同步尾部，且会引入更强的优化漂移风险 @stich2019local。本章提出在通信受限场景下的混合并行通信掩盖策略（本文简称其为 POLAR-SGD）：在不改变“每迭代一次全局同步”这一基本语义的前提下，通过 *前缀触发的异步 All-Reduce + 预测误差修正* ，创造了一个*更早、更长*的可重叠窗口（在 micro-batch 前缀处触发一次 all-reduce 集合通信），让整个跨 WAN 的 All-Reduce 得以被后续反向传播的通信所掩盖，从而将同步从阻塞尾部中去除。

== 问题分析与研究思路

考虑一个 DP+PP 混合训练过程：为减少显存负载压力并缓解各阶段 GPU 气泡（Bubble），系统通常将一个全局迭代（step）的单个 mini-batch 进一步切分为 $M$ 个更细粒度的 micro-batch（微批次）。在执行引擎层面，常采用 1F1B（One-Forward-One-Backward）调度执行，即打破单纯的串行执行逻辑，令处于流水线前后的设备交替执行前向与反向传播。但在单次迭代的末尾，这种流水线必须经历一个排空阶段（Flush），不再下发新的数据批次，最终使得所有设备退避到同步状态。
...
在该设定下，域 内 PP 负责把模型切分成多层以提升显存可承载的模型规模，并通过 1F1B 将多个 micro-batch 的前向/反向交错执行来提高单 域 内的流水线利用率；跨 域 DP 则要求每步获得一致的更新方向，需要对 DP 组内的梯度做均值 All-Reduce。该 All-Reduce 因 WAN 特性成为尾部瓶颈。
...
@fig:polar-overview 对比了标准 DP+PP 与 POLAR-SGD 的单次迭代时间线，可以看到POLAR-SGD 通过前缀触发的异步 All-Reduce 缩短同步尾部，每一行代表着在各个 DP 组内的各个 DP 进程中的对等 PP rank 的行为。其中，红色块代表一个微批次的前向传播，白色块代表一个微批次的反向传播，紫色块代表在反向传播结束后的梯度通信同步。标准做法往往在处理完全部 $M$ 个 micro-batch 的反向传播后，才在迭代尾部执行阻塞式 All-Reduce，形成明显同步尾部；POLAR-SGD 则在 micro-batch 前缀完成时（$m=tau$）触发 *一次* 非阻塞 All-Reduce，并放入独立通信 stream，使其与后续 $(M-tau)$ 个 micro-batch 的反向传播重叠，从而缩短迭代尾部。

#figure(
  image("../../image/polar-pp-timeline.png", width: 98%),
...
令 $T_"ar"$ 表示本次梯度 All-Reduce 的平均通信时延估计，$T_"comp"(tau)$ 表示在 $m=tau$ 触发通信后，迭代内剩余的计算时间（通常来自后续 micro-batch 的反向传播）。本文采用*最早可完全掩盖*的截断点：
...
虽然本章重点在算法机制，但要在真实的 DP+PP 系统中获得稳定收益，还需要在控制流与执行资源（CUDA stream）层面保证异步通信确实与计算并行。这里给出更贴近工程实现的集成要点。

=== 与 1F1B 流水线的集成位置

在 1F1B 调度中，micro-batch 的反向传播会按固定节奏在各 stage 上完成。POLAR-SGD 的关键是：在每个 DP rank 上，在某个 micro-batch 的反向结束时刻统一触发一次 All-Reduce。实现时通常会在梯度累积完成处设置钩子函数（hook）（或在训练循环里显式判断 $m=tau$），以便在该时刻将 $s_(r,t)$ 交给通信后端。

由于各 stage 的反向完成存在相位差，实际系统中更常见的做法是：在每个 DP rank 内先对所有参数做一次完整梯度张量的累积（或以 bucket 形式累积），然后在 $m=tau$ 时对该累积缓冲做快照并外推梯度，而后进入 All-Reduce 通信环节。这样能避免在每层/每 bucket 上重复触发通信，符合每个训练迭代执行一次 All-reduce 梯度同步的设计初衷。

=== 通信流与计算流的隔离

要实现重叠，All-Reduce 必须在独立的通信流上启动，且计算流不应等待其完成。一个常见实现骨架是：

其典型执行链路是：计算流在 micro-batch $m=tau$ 反向结束后先记录 event；通信流等待该 event 以确保 $s_(r,t)$ 写入完成后，再启动异步 All-Reduce（如 NCCL async_op）；随后计算流继续处理后续 micro-batch 的反向传播和梯度累积，直到 $m=M$ 的更新点才显式等待通信句柄并获取 $g_"sync",t$。
