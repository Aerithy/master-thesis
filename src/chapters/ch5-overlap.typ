...
本文在跨域训练约束下评估 POLAR-SGD，并与标准 DP+PP 基线（DDP+1F1B，迭代末阻塞 All-Reduce）以及放松同步的 LSGD（$k=4$）对比。实验配置如下。
...
消融结果强调了*梯度缩放（Gradient Scaling）与误差反馈（Error Feedback）*两个机制的必要性：一方面，若不执行 $(M/tau)$ 缩放，同步梯度尺度会系统性偏小，导致更新幅度不足并拖慢收敛；若不维护 residual，后缀 micro-batch 的信息就难以在后续迭代中有序回流，进而增加收敛退化与稳定性风险。
...
综合吞吐、收敛与消融三组证据，方法在系统侧缩短尾部等待，在优化侧保持轨迹稳定，在机制层具备可解释的必要性。因此，本章关于 POLAR-SGD 在跨域 DP+PP 场景中的适用性结论具备充分实验支撑。
