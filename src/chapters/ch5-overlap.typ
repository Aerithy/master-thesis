...
@fig:polar-overview 对比了标准 DP+PP 与 POLAR-SGD 的单次迭代时间线。标准做法往往在处理完全部 $M$ 个 micro-batch 的反向传播后，才在迭代尾部执行阻塞式 All-Reduce，形成明显同步尾部；POLAR-SGD 则在 micro-batch 前缀完成时（$m=tau$）触发 *一次* 非阻塞 All-Reduce，并放入独立通信 stream，使其与后续 $(M-tau)$ 个 micro-batch 的反向传播重叠，从而缩短迭代尾部。
...
从关键路径角度看，吞吐提升并非仅由通信总量变化驱动，也由“尾部阻塞长度”缩短驱动。即使总通信字节不变，只要将 All-Reduce 前移到可重叠窗口，迭代末端的纯等待区间就会明显缩短。表格中的相对加速比正体现了这一点：优化对象是迭代尾部而非全程任意位置的通信片段，因此收益具有明确因果指向。
