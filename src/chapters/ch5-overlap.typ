...
在 WAN 延迟主导时，即使框架在反向传播期间做了梯度分桶来使梯度同步 All-Reduce 与计算重叠，也容易在每次训练迭代末尾出现无法被剩余计算掩盖的通信同步阻塞尾部。原因在于，WAN 的 $T_"ar"$ 可能大于单个 bucket 之后剩余的可用计算窗口，导致最后几个 bucket 的同步受限于串行堆叠，形成长尾。@fig:dppp-timebreakdown 展示了 DP+PP 配置下跨 DC 与 DC 内开销分解：跨 DC 同步在每步中占据一定比例，导致 GPU 等待。
...
面对同步尾部，一类直观做法是弱化同步（如 Local-SGD），该类做法通过在各个数据中心内部维护一个局部模型，并通过约定好的同步频率来维护各个数据中心的模型参数一致，但这类方法的本质是降低同步频率而非消除同步尾部，且会引入更强的优化漂移风险 @stich2019local。本章提出在通信受限场景下的混合并行通信掩盖策略（本文简称其为 POLAR-SGD）：在不改变“每迭代一次全局同步”这一基本语义的前提下，通过 *前缀触发的异步 All-Reduce + 预测误差修正* ，创造了一个*更早、更长*的可重叠窗口（在 micro-batch 前缀处触发一次 all-reduce 集合通信），让整个跨 WAN 的 All-Reduce 得以被后续反向传播的通信所掩盖，从而将同步从阻塞尾部中去除。
...
在本章的实验拓扑映射中，跨 DC 使用 DP（DP 组大小为 $N$），同一 DC 内使用 PP（用于模型切分与计算流水线）。
...
POLAR-SGD 的设计可概括为“系统效率、语义约束、优化稳定性”三者并重：在系统层面尽可能将跨 DC All-Reduce 前移到更早位置，以获得更长的计算重叠窗口并缩短迭代尾部；在语义层面保持“每迭代一次 DP 同步”的宏观节奏，避免完全异步导致的严重的梯度滞后（staleness）；在优化层面通过可控残差通道补齐后缀梯度信息，使 step-wise 收敛轨迹尽量贴近基线同步训练。
...
前缀触发意味着本次迭代的同步更新方向来自前缀梯度。为避免后缀 micro-batch 的梯度信息被忽略，POLAR-SGD 通过“前缀缩放”构造全 batch 尺度的预测梯度，并维护误差缓冲记录预测与真实完整累积之间的偏差。
...
从直观上看，$(M/tau)$ 的缩放因子承担了“把前缀的梯度尺度外推到全 batch”的作用：如果梯度的统计分布在 micro-batch 维度上相对平稳，那么 $g_(r,t)^(tau)$ 的期望规模与 $tau$ 成正比，乘以 $(M/tau)$ 后就与 $g_(r,t)^(M)$ 更接近。误差缓冲 $e_(r,t)$ 则用于吸收“外推偏差 + 后缀梯度信息”，把这些信息以残差（residual）的形式带到下一步同步中。

此外，本文给出了一个合理的“守恒”视角：对 DP 组内取平均，令 $bar g_t^(M) := (1/N) sum_(r=1)^N g_(r,t)^(M)$ 且 $bar e_t := (1/N) sum_(r=1)^N e_(r,t)$，则在误差更新定义下可得到

$ g_"sync",t + bar e_(t+1) = bar g_t^(M) $

该等式说明：同步更新方向与下一步的平均残差共同分解了“本步完整累积梯度”的信息；也就是说，后缀 micro-batch 的信息并未被忽略，只是被延迟到下一迭代中体现。
...
在 1F1B 调度中，micro-batch 的反向传播会按固定节奏在各 stage 上完成。POLAR-SGD 的关键是：在每个 DP rank 上，在 *某个 micro-batch 的反向结束时刻* 统一触发一次 All-Reduce。实现时通常会在“梯度累积完成事件”处设置钩子函数（hook）（或在训练循环里显式判断 $m=tau$），以便在该时刻将 $s_(r,t)$ 交给通信后端。
...
其典型执行链路是：计算 stream 在 micro-batch $m=tau$ 反向结束后先记录 event；通信 stream 等待该 event 以确保 $s_(r,t)$ 写入完成后，再启动异步 All-Reduce（如 NCCL async_op）；随后计算 stream 继续处理后续 micro-batch 的反向传播和梯度累积，直到 $m=M$ 的更新点才显式等待通信句柄并获取 $g_"sync",t$。
...
从吞吐对比可以看出：在 Cross-DC 约束下，基线 DDP+1F1B 的尾部同步显著拉长了单步时间；DiLoco 通过减少同步强度/频率获得了较大吞吐提升，但其优化语义偏离基线更明显；POLAR-SGD 在保持“每步一次同步”的节奏下仍获得最高吞吐，约为基线的 1.87 倍，说明将同步从尾部挪到可重叠窗口比减少同步频率更契合 DP+PP 的系统结构。
...
本文进一步给出了按 step 对齐的 loss 曲线（验证 step-wise 稳定性）、按 wall-clock time 对齐的 loss 曲线（验证目标 loss 到达时间，即 time-to-target 提升），以及对“梯度缩放 / 误差反馈”两项关键机制的消融结果。
...
消融实验则从反证角度补强结论。去除缩放后出现尺度偏差，去除误差反馈后后缀信息回流不足，两者都导致收敛性能劣化，说明 POLAR-SGD 的有效性来自“时机重排 + 误差控制”两部分的协同作用。该结果直接支撑本章结论中“机制缺一不可”的表述。

综合吞吐、收敛与消融三组证据，可以形成完整论证闭环：方法在系统侧缩短尾部等待，在优化侧保持轨迹稳定，在机制层具备可解释的必要性。因此，本章关于 POLAR-SGD 在跨域 DP+PP 场景中的适用性结论具备充分实验支撑。

== 本章小结

本章针对 Cross-DC 条件下 DP+PP 训练的同步尾部问题，提出并验证了 POLAR-SGD 的通信掩盖方案。WAN 高 RTT 使得迭代末 All-Reduce 成为关键路径瓶颈，常规分桶重叠难以覆盖尾部阻塞。于是，本章采用“跨 DC DP、DC 内 PP”的层次化并行骨架，将跨域通信限定为梯度同步；在每迭代只触发一次 All-Reduce 的约束下，引入截断点 $tau$ 与独立通信 stream 提前触发异步同步，并通过前缀缩放与误差反馈机制补齐后缀梯度信息。最终，在 30 Gb/s、50 ms RTT 的 Cross-DC 约束下，方法获得约 1.87× 的吞吐提升，并保持与基线接近的收敛轨迹；消融结果进一步证明梯度缩放与误差反馈均为稳定性与效果的关键组成。综上，本章在不改变“每步一次同步”语义的前提下，通过同步时机重排与误差修正有效削减尾部阻塞。
