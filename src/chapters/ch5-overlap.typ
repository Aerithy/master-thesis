...
结果显示，梯度缩放与误差反馈两项机制缺一不可：若不执行 $(M/tau)$ 缩放，同步梯度尺度会系统性偏小，导致更新幅度不足并拖慢收敛；若不维护 residual，后缀 micro-batch 的信息就难以在后续迭代中有序回流，进而增加收敛退化与稳定性风险。
...
本章围绕 Cross-DC 下 DP+PP 训练的同步尾部问题给出了完整方案：首先采用“跨 DC DP、DC 内 PP”的层次化并行骨架，避免跨 DC 激活和样本搬运；随后在“每迭代只触发一次 All-Reduce”的约束下，引入截断点 $tau$ 与独立通信 stream，使同步与后续反向传播产生可观重叠并缩短尾部；最后通过前缀缩放与误差反馈机制，将后缀 micro-batch 信息以残差通道延迟注入下一迭代，在系统效率与优化稳定性之间实现平衡。实验在 30 Gb/s、50 ms RTT 的 Cross-DC 约束下取得约 1.87× 的吞吐提升，同时保持与基线接近的 step-wise 收敛轨迹。
