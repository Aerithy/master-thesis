...
在分布式深度学习训练中，梯度同步的通信开销已成为制约训练效率的关键瓶颈 @pytorch_distributed2020。特别是在多节点集群环境中，通信架构表现出明显的异构性（Heterogeneity）：节点内（Intra-node）通信通过高速 NVLink 或 PCIe 完成，带宽可达数百 GB/s；而节点间（Inter-node）通信受限于以太网或 InfiniBand 带宽，通常仅为 10-100 Gbps @nvidia_nvlink_v3_2021 @mellanox_hdr200。这种显著的带宽不对称性（Bandwidth Asymmetry），通常达到 10:1 甚至 50:1。这使得跨节点通信成为系统的主要性能短板。
