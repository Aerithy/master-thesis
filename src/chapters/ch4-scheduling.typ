...
在分布式深度学习训练中，梯度同步的通信开销已成为制约训练效率的关键瓶颈 @pytorch_distributed2020。特别是在多节点集群环境中，通信架构表现出明显的异构性：节点内（Intra-node）通信通过高速 NVLink 或 PCIe 完成，带宽可达数百 GB/s；而节点间（Inter-node）通信受限于以太网或 InfiniBand 带宽，通常仅为 100-200 Gbps #multicite[@nvidia_nvlink_v3_2021 @mellanox_hdr200]。这种显著的带宽不对称性（Bandwidth Asymmetry）可能达到 10:1。这使得跨节点通信成为系统的主要性能短板。
...
额外内存开销分析：额外开销主要来自 CUDA Event 对象管理，其规模约为 $O(N_"chunks")$；而张量分块本身采用视图操作，不引入新的实质性数据拷贝。PyTorch 等框架中的 Tensor Chunk 仅创建新的 Metadata 视图（View）而不拷贝底层显存，因此对动辄数十 GB 的模型梯度而言，仅有的几十个轻量级 CUDA Event 对象所占用的系统内存几乎可以忽略不计。因此总体空间复杂度仍为 $O(N_"chunks")$，在典型分块数（4-8）下可视为工程上可忽略的附加成本。
...
最后，系统保留了原生扁平通信的后备降级路径。可以通过配置开关在并行流水线调度与传统的 `all_reduce` 之间进行运行时切换。这种透明化集成策略将通信协议管理的复杂度限制在底层引擎，对上层用户训练脚本而言并无侵入性。
...
本章实验涉及两类硬件环境。实验环境一为 NVIDIA A100 云上双节点集群，节点内 GPU 通过 PCIe 全互联，网卡使用 IB 网口；节点内 PCIe 有效带宽约为 126 Gb/s，节点间互联带宽约为 50 Gb/s。软件侧采用 PyTorch/NCCL/CUDA 分布式训练栈，并固定通信后端、batch 配置和日志采集脚本，以保证不同通信策略之间仅在集合通信路径与调度方式上存在差异。任务侧采用 ResNet-50 与 ImageNet 组合，并设置每 GPU batch 为 256 #multicite[@he2016resnet @deng2009imagenet]。对比对象包括 PyTorch 原生 All-Reduce 与无流水线的朴素层次化 All-Reduce。

实验环境二为天数智芯（Iluvatar CoreX）国产 GPU 物理集群，节点 n210 对应 Domain-1，节点 n62 对应 Domain-2；每节点 CPU 为 Intel Xeon Gold 6330，GPU 为 Iluvatar BI-V150 x 8，板卡内部各包含 2 个 GPU 芯粒。该环境中 GPU 两两成对走板内集成互联路径，每对 GPU 之间经 PCIe Switch 互联且不跨 NUMA，所有 GPU 位于同一 NUMA，NIC0 挂载在另一 NUMA 节点上；节点间通过 Ethernet 互联，节点内全 PCIe 互联带宽约为 126 Gb/s，节点间互联带宽约为 12.5 Gb/s。由于该环境的通信后端、网卡位置与节点间链路均不同于 A100 云集群，本章将其作为异构硬件可迁移性与边界条件验证。
...
为体现本文方法在跨域互联中的适用性，实验还采用网络整形对应的域间链路注入带宽约束，用于浮现训练时的广域网特征；域内链路仍保持域内高带宽互联，以刻画典型的域内快、域间慢的云边端跨域训练环境。该设置重点用于验证层次化流水线在跨域瓶颈下的调度收益与敏感性趋势。
...
这一结果可以从拓扑匹配 + 时序重叠两个维度解释。朴素层次化主要解决的是拓扑不匹配问题，即将经由慢链路通信的数据块数降低从而降低由于高RTT和低带宽导致的通信效率低的问题；流水线层次化则进一步重排执行时序，把本地归约、跨节点归约和本地广播并行化，从而进一步提升了在整体网络环境下的带宽利用率。

GPU 利用率提升与通信耗时下降是因为，同步通信缩短后，计算流等待通信完成的空转时间减少，GPU 能够更连续地执行前后向计算。由此，通信层优化直接转化为计算资源利用效率提升，从而可以提升整体训练效率。
...
端到端结果进一步验证了通信优化能够转化为训练层面的实质收益。相较通信耗时指标，迭代时间和吞吐更接近业务目标，因此其改善更具说服力。数据表明方法从减少通信等待推进到缩短训练迭代。
...
此外，朴素层次化与流水线层次化形成了清晰的递进关系，避免了仅与单一基线比较导致的解释歧义。先证明拓扑分层有效，再证明时序流水化在分层基础上继续增益，使结论链条更完整：收益来源可分解、可解释、可复现。

=== 块大小敏感性分析

@fig:chunk-sensitivity 展示了不同块大小对性能的影响。

敏感性结果显示，块数在 4-8 区间时性能最优，对应块大小约为张量尺寸的 $1/4 tilde 1/8$；当块数低于 4 时，并行深度不足，通信难以被充分掩盖；而当块数增加到 16 时，启动开销与事件同步开销已经超过流水线重叠收益，开始出现负优化。
...
变量 2（通信张量大小）：固定为 256 MB；
变量 3（分块数）：1、2、4、8（对应不同 chunk 粒度）。

#h(0em) 结果表明：流水线层次化方案在轻度到中等带宽不对称场景中收益最明显；当节点内/节点间带宽比继续增大时，节点间慢链路逐渐成为不可隐藏的主瓶颈，加速比随之收敛到 1 附近。在固定 256 MB 通信张量下，chunk=4 取得整体最优结果，chunk=8 与其接近但在极端失衡时受到调度开销影响，而 chunk=1 受并行深度不足限制。该现象表明分块粒度与网络条件存在显著耦合关系，合理选择 chunk 数是低质网络优化的关键。

带宽不对称实验给出了本章方法适用性的关键证据。若方法仅在近似同构网络下有效，则其跨域价值有限；当前结果显示，在 2:1 等轻度失衡场景中流水线重叠能够显著缩短等待时间，而在 20:1、30:1 等极端失衡场景中收益逐步收敛，说明层次化流水线能够缓解但不能完全消除跨域慢链路瓶颈。
...
为进一步验证层次化集合通信调度在异构国产 GPU 物理集群中的行为，本章在实验环境二上进行了 All-Reduce 微基准实验。该实验保持两节点、每节点 4 GPU 的分布式进程布局，即在每个物理节点中选取 4 张 GPU 参与测试，以对齐实验环境一的 8 进程规模。实验比较 PyTorch/NCCL 原生 All-Reduce、朴素层次化 All-Reduce 与流水线层次化 All-Reduce 三种路径，并在 4 MB 至 128 MB 的张量规模下统计平均通信耗时与有效吞吐。

从@tab:corex-collective-latency 和 @tab:corex-collective-throughput 可以看到，在 CoreX 物理集群中，层次化流水线的收益随张量规模增大而逐渐显现。对于 4 MB 与 16 MB 小张量，PyTorch/NCCL 原生路径分别为 1.56 ms 与 4.73 ms，仍略优于流水线层次化；这是因为小张量下通信启动、进程组调度与事件同步开销占比较高，流水线尚未形成足够长的重叠窗口。对于 64 MB 和 128 MB 张量，流水线层次化分别将耗时降至 12.31 ms 和 21.12 ms，相比原生 All-Reduce 的 17.08 ms 和 33.28 ms 明显更优；其中 128 MB 场景下有效吞吐由 4.03 GB/s 提升至 6.35 GB/s，约为 $1.58 times$。
...
该补充实验为第四章结论提供了更明确的适用边界：层次化流水线更适合中大张量和跨节点通信占比较高的同步阶段，而对于小张量通信，原生 collective 的低启动开销仍具有优势。因此，在国产 GPU 集群等新后端上部署时，不应静态替换所有 All-Reduce 操作，而应根据张量规模、拓扑结构和在线基准测试结果选择通信路径。对于大模型训练中常见的大梯度桶，实验环境二的结果仍支持本文关于“域内快、域间慢”拓扑下采用流水线层次化调度的核心结论。
