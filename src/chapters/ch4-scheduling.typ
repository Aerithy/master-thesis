...
在分布式深度学习训练中，梯度同步的通信开销已成为制约训练效率的关键瓶颈 @pytorch_distributed2020。特别是在多节点集群环境中，通信架构表现出明显的异构性：节点内（Intra-node）通信通过高速 NVLink 或 PCIe 完成，带宽可达数百 GB/s；而节点间（Inter-node）通信受限于以太网或 InfiniBand 带宽，通常仅为 100-200 Gbps @nvidia_nvlink_v3_2021 @mellanox_hdr200。这种显著的带宽不对称性（Bandwidth Asymmetry）可能达到 10:1。这使得跨节点通信成为系统的主要性能短板。
...
另一方面，现有通信优化方案在应对异构拓扑时存在适应性局限。梯度压缩类方法 @dgc2018 @bytepscompress2021 通过降低传输数据量缓解带宽压力，关键的是，这些上述方法大多将集合通信视为原子操作进行整体优化，未能挖掘在集合通信内部，存在域内 - 域间通信并行的潜力。
...
实验在以下环境中进行：实验平台采用 2 个计算节点、每节点 4 张 NVIDIA A100 GPU；节点内互联为 NVLink（600 GB/s），节点间互联为 InfiniBand（200 Gbps）@nvidia_a100_2020 @nvidia_nvlink_v3_2021 @mellanox_hdr200。任务侧采用 ResNet-50 与 ImageNet 组合，并设置每 GPU batch 为 256 @he2016resnet @deng2009imagenet。对比对象包括 PyTorch 原生 All-Reduce 与无流水线的朴素层次化 All-Reduce。
...
敏感性结果显示，块数在 8-16 区间时性能最优，对应块大小约为张量尺寸的 $1/8 tilde 1/16$；当块数低于 8 时，并行深度不足，通信难以被充分掩盖；而当块数超过 32 时，启动开销累积主导，整体性能反而下降。
