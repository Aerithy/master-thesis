...
云-边-端协同训练在网络层形成明显的多层次带宽与时延差异。域内链路方面，同一节点内的 NVLink 互联带宽可达 600 GB/s，节点间高性能以太网或 InfiniBand HDR RDMA 可提供 200 Gb/s 以上的低时延互联；而跨域链路方面，典型的边缘↔云或边缘↔边缘的 WAN 带宽通常仅有 1–100 Gb/s，RTT 在数十至数百毫秒量级，且受网络拥塞与路由波动影响，抖动明显 @nvidia_nvlink_v3_2021 @mellanox_hdr200 @jain2013b4。这种"域内高速—域间受限"的分层互联特征，使得传统上为同质高速网络设计的扁平化集合通信策略（如 Ring All-Reduce）难以有效适配 @rabenseifner2004，梯度同步极易进入迭代关键路径，形成显著的通信瓶颈 @yu2023cross。
...
*挑战二：高 RTT 与链路抖动加剧同步尾部延迟。* WAN 链路的高 RTT（跨大洲可达 50–200 ms）与抖动（10–50 ms 量级）会将集合通信的启动开销与同步等待放大至不可忽略的程度 @jain2013b4。在同步 DP 框架中，每步训练须等待最慢节点完成通信（"木桶效应"），而链路抖动带来的随机慢节点（straggler）效应会使吞吐分布出现长尾，系统整体有效吞吐受最慢单次通信主导。在 DP+PP 混合并行场景中，流水线末尾阶段的 All-Reduce 本已处于计算关键路径之外，高 RTT 进一步扩大尾部阻塞窗口，加重设备空闲 @crosspipe2025 @geopipe2025。
...
为便于刻画云边端协同的跨域网络特征并开展可控实验，本文以跨数据中心训练作为典型强约束实例：跨 DC 场景的 WAN 带宽与 RTT 特征与边缘↔云或边缘↔边缘的典型互联相吻合，且可在实验中通过流量整形工具（tc-netem）精确复现，便于系统性评估各优化手段的收益 @hemminger2005netem。
