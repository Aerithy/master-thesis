...
本节从“数据中心内高带宽”和“云边端跨域低带宽”两类环境设置实验，以体现量化方法在云边端协同中的适用性。实验关注通信量、收敛稳定性与端到端吞吐三类指标。

#h(-2em) *环境 A：局域高带宽集群（对照）*：
- 目的：验证 1-bit 量化在常规分布式训练中的通信压缩与训练稳定性
- 拓扑：同一机房/同一数据中心内多机多卡（典型高带宽、低 RTT）

#h(-2em) *环境 B：云-边-端协同链路（跨域约束）*：
- 目的：评估在带宽受限、RTT 较高的跨域链路下，量化对通信尾部与整体吞吐的改善潜力
- 拓扑抽象：端侧（Device）→ 边缘（Edge）→ 云（Cloud）三层；其中端到边/边到云链路为主要瓶颈
- 复现方式：通过网络整形（限速 + RTT 注入 + 抖动）在集群上模拟跨域链路特征，使得端/边参与的梯度同步需跨受限链路传输
...
#h(-2em) *模型与任务*：
- 视觉模型（如 ResNet-50）与语言模型（如 BERT-Large）作为代表性工作负载

#h(-2em) *对比基线*：
- FP32 / FP16 的常规 All-Reduce 同步
- INT8/FP16 等低精度压缩（若启用）

#h(-2em) *评价指标*：
- 通信量（每步传输字节数）与通信耗时（含尾部等待）
- 吞吐（samples/s 或 tokens/s）
- 收敛曲线（loss vs steps 与 loss vs time）
