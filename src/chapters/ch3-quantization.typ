...
在分布式深度学习训练中，梯度通信开销已成为制约训练效率的关键瓶颈。传统的 AllReduce 操作需要在各个计算节点间传输完整的 32 位浮点梯度，在大规模模型和多节点环境下，通信时间往往占据训练过程的主要部分。为降低通信开销，本文引入了 *k-bit随机舍入量化方法*，将梯度压缩至可配置的低比特表示（$b in {1, 2, 4, 8, 16}$），可在压缩率与收敛稳定性之间进行灵活折中。
...
相比之下，本文采用的 k-bit 量化方法结合了 Adam 风格的动量归一化与随机舍入策略，能够在保证收敛性的同时支持多档压缩精度：1-bit 强压缩、2/4-bit 均衡压缩、8/16-bit 低扰动压缩。
...
#figure(
  table(
    ...
    stroke: none,
    table.hline(),
    table.header([*算法 1：k-bit随机舍入梯度量化算法*]),
